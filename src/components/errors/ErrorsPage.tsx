...
export function ErrorsPage() {
  ...
  const [cpanelErrorDetails, setCpanelErrorDetails] = useState(
        ...
        <div className="max-w-full mx-auto">
          ...
          {activeTab === "logger" && (
            <div className="space-y-6">
              {Object.entries(groupLoggerErrors()).map(
                ([application, errors]) => (
                  <div
                    key={application}
                          ...
                          {errors.map((error) => (
                            ...
                            >
                              <td className="px-6 py-4 whitespace-nowrap">
                                <span
                                  className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getLevelColor(error.level)}`}
                                >
          ...
          {activeTab === "cpanel-details" && (
            <div className="space-y-6">
              {Object.entries(groupCpanelErrorDetails()).map(
                ([directory, errors]) => (
                  <div
                    key={directory}
                    className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden"
                            ...
                            >
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                <div className="flex items-center space-x-1 text-sm text-gray-900 dark:text-white">
                                  <FileText className="w-4 h-4 text-gray-400" />
                                  <span
                                    className="font-mono text-xs bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded-sm max-w-xs truncate"
                                    title={error.errorFile}
                                  >
